 ...
 * return the audio for each sentence between one Flush and the next, so each sentence's
 * length in bytes is known once it is flushed. Within a sentence the time is shared out
 * over the words by their length and the pauses after them. The play head resynchronises
 * at every sentence, so an estimate never drifts further than the sentence it is in.
 *
...
class SpeechClock {
    ...
    fun sentence(text: String) {
        ...
        synchronized(this) {
            sentences.add(Sentence(wordTotal, weights.size, chars, cumulative))
            wordTotal += weights.size
        }
    }

    /** Audio bytes arrived; they belong to the oldest sentence not yet flushed. */
    fun audio(bytes: Int) {
        synchronized(this) {
            totalBytes += bytes
    ...
    /** The voice finished one sentence's audio. */
    fun flushed() {
        synchronized(this) {
            val s = sentences.getOrNull(open) ?: return
            s.flushed = true
     ...
     * heard. Returns true when the word or the position within it moved.
     */
    fun seek(playedBytes: Long): Boolean {
        ...
        synchronized(this) {
            ...
            var found = false
            for (s in sentences) {
                val len = if (s.flushed) s.bytes else maxOf(s.bytes, s.chars * BYTES_PER_CHAR)
    ...
    private companion object {
        /** A rough length for a sentence still arriving: 24 kHz 16-bit speech runs near sixteen characters a second. */
